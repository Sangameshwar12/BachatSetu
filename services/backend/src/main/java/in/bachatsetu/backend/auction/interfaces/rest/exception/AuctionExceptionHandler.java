package in.bachatsetu.backend.auction.interfaces.rest.exception;
...
/** Converts Auction REST boundary failures to RFC 7807 problem details. */
@RestControllerAdvice(basePackages = "in.bachatsetu.backend.auction.interfaces.rest")
public class AuctionExceptionHandler {
    ...
    ResponseEntity<ProblemDetail> handleAuctionNotFound(
    ...
    @ExceptionHandler({AuctionAccessDeniedException.class, DrawAccessDeniedException.class})
    ResponseEntity<ProblemDetail> handleAccessDenied(RuntimeException exception, HttpServletRequest request) {
        return response(
    ...
    ResponseEntity<ProblemDetail> handleMemberNotEligible(
            MemberNotEligibleException exception,
            HttpServletRequest request) {
        return response(
    ...
    ResponseEntity<ProblemDetail> handleInvalidBidAmount(
            InvalidBidAmountException exception,
            HttpServletRequest request) {
        return response(
                HttpStatus.UNPROCESSABLE_ENTITY,
                "invalid-bid-amount",
    ...
    ResponseEntity<ProblemDetail> handleUnexpected(Exception exception, HttpServletRequest request) {
        LOGGER.error("Unhandled auction API failure for {}", request.getRequestURI(), exception);
        return response(
