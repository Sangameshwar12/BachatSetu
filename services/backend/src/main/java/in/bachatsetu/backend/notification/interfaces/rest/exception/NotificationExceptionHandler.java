package in.bachatsetu.backend.notification.interfaces.rest.exception;
...
/** Converts Notification REST boundary failures to RFC 7807 problem details. */
@RestControllerAdvice(basePackages = "in.bachatsetu.backend.notification.interfaces.rest")
public class NotificationExceptionHandler {
    ...
    ResponseEntity<ProblemDetail> handleNotificationNotFound(
    ...
    ResponseEntity<ProblemDetail> handleDeliveryFailed(
            NotificationDeliveryFailedException exception,
            HttpServletRequest request) {
        return response(
    ...
    ResponseEntity<ProblemDetail> handleUnexpected(Exception exception, HttpServletRequest request) {
        LOGGER.error("Unhandled notification API failure for {}", request.getRequestURI(), exception);
        return response(
