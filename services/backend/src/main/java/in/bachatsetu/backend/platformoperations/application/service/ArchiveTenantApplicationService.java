...
/**
 * Records a {@code TENANT_ARCHIVED} audit entry directly rather than through an Audit event listener: see
 * {@link SuspendTenantApplicationService}'s Javadoc for why.
 */
public final class ArchiveTenantApplicationService implements ArchiveTenantUseCase {
    ...
    public ArchiveTenantApplicationService(
    ...
    @Override
    public TenantResult execute(ArchiveTenantCommand command) {
        ...
        TenantResult result = transaction.execute(() -> {
            ...
            tenant.archive(command.actorId(), clock.now());
        ...
        auditTenantArchived(command);
        return result;
    }

    private void auditTenantArchived(ArchiveTenantCommand command) {
        try {
            createAuditEntry.execute(new CreateAuditEntryCommand(
                    command.tenantId(), command.actorId(), AuditEventType.TENANT_ARCHIVED, "platformoperations",
                    "Tenant", command.tenantId(), "TENANT_ARCHIVED", "archived a tenant", null, null, null));
        } catch (RuntimeException exception) {
            // Audit is best-effort: never let a recording failure affect an already-applied archival.
        }
    }
}
