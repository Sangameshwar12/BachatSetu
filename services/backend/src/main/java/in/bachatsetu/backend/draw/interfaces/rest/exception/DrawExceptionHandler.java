...
/** Converts Draw REST boundary failures to RFC 7807 problem details. */
@RestControllerAdvice(basePackages = "in.bachatsetu.backend.draw.interfaces.rest")
public class DrawExceptionHandler {

    private static final Logger LOGGER = LoggerFactory.getLogger(DrawExceptionHandler.class);
    ...
    ResponseEntity<ProblemDetail> handleDrawNotFound(
            DrawNotFoundException exception,
            HttpServletRequest request) {
        return response(
    ...
    ResponseEntity<ProblemDetail> handleUnexpected(Exception exception, HttpServletRequest request) {
        LOGGER.error("Unhandled draw API failure for {}", request.getRequestURI(), exception);
        return response(
