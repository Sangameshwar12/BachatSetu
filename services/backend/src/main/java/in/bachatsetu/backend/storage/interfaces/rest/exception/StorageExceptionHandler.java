package in.bachatsetu.backend.storage.interfaces.rest.exception;
...
@RestControllerAdvice(basePackages = "in.bachatsetu.backend.storage.interfaces.rest")
public class StorageExceptionHandler {
    ...
    ResponseEntity<ProblemDetail> handleNotFound(StoredFileNotFoundException exception, HttpServletRequest request) {
        return response(HttpStatus.NOT_FOUND, "not-found", "Resource not found", exception.getMessage(), request);
    }

    @ExceptionHandler({UnsupportedStorageProviderException.class, DomainException.class})
    ResponseEntity<ProblemDetail> handleBusinessFailure(RuntimeException exception, HttpServletRequest request) {
        return response(
                HttpStatus.UNPROCESSABLE_ENTITY, "storage-validation-failed", "Business validation failed",
    ...
    ResponseEntity<ProblemDetail> handleUnexpected(Exception exception, HttpServletRequest request) {
        LOGGER.error("Unhandled storage API failure for {}", request.getRequestURI(), exception);
        return response(
