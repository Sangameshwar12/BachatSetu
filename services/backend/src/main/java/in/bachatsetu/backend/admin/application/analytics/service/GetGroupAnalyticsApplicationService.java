...
/** Computes savings group analytics, then best-effort records {@code ADMIN_ANALYTICS_VIEWED}. */
public final class GetGroupAnalyticsApplicationService implements GetGroupAnalyticsUseCase {

    private final GroupAnalyticsRepository repository;
    private final TransactionPort transaction;
    private final AnalyticsApplicationMapper mapper;
    private final CreateAuditEntryUseCase createAuditEntry;

    public GetGroupAnalyticsApplicationService(
    ...
    @Override
    public GroupAnalyticsResult execute(ViewAnalyticsCommand command) {
        Objects.requireNonNull(command, "view analytics command must not be null");
        GroupAnalyticsResult result = transaction.execute(() -> mapper.toResult(repository.compute()));
    ...
    private void auditAnalyticsViewed(AggregateId administratorId) {
        try {
            createAuditEntry.execute(new CreateAuditEntryCommand(
                    ...
                    "ADMIN_ANALYTICS_VIEWED", "viewed group analytics", null, null, "{\"view\":\"groups\"}"));
