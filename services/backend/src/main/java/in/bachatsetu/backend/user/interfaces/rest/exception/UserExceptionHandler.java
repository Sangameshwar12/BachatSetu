package in.bachatsetu.backend.user.interfaces.rest.exception;
...
/** Converts user module REST boundary failures to RFC 7807 problem details. */
@RestControllerAdvice(basePackages = "in.bachatsetu.backend.user.interfaces.rest")
public class UserExceptionHandler {
    ...
    @ExceptionHandler(OnboardingApplicationException.class)
    ResponseEntity<ProblemDetail> handleOnboardingFailure(
            OnboardingApplicationException exception, HttpServletRequest request) {
        return switch (exception.reason()) {
            case PROFILE_NOT_FOUND -> response(
                    HttpStatus.NOT_FOUND, "profile-not-found", "Profile not found", exception.getMessage(), request);
            case ALREADY_ONBOARDED -> response(
                    HttpStatus.CONFLICT, "already-onboarded", "Onboarding already completed", exception.getMessage(),
                    request);
        };
    }
    ...
    ResponseEntity<ProblemDetail> handleUnexpected(Exception exception, HttpServletRequest request) {
        LOGGER.error("Unhandled user API failure for {}", request.getRequestURI(), exception);
        return response(
