package in.bachatsetu.backend.member.interfaces.rest.exception;
...
/** Converts Member REST boundary failures to RFC 7807 problem details. */
@RestControllerAdvice(basePackages = "in.bachatsetu.backend.member.interfaces.rest")
public class MemberExceptionHandler {
    ...
    @ExceptionHandler(DuplicateMemberNumberException.class)
    ResponseEntity<ProblemDetail> handleDuplicateNumber(
            DuplicateMemberNumberException exception,
            HttpServletRequest request) {
        return response(
                HttpStatus.CONFLICT,
    ...
    ResponseEntity<ProblemDetail> handleMemberNotFound(
    ...
    @ExceptionHandler(DuplicateGroupParticipationException.class)
    ResponseEntity<ProblemDetail> handleDuplicateParticipation(
            DuplicateGroupParticipationException exception,
            HttpServletRequest request) {
        return response(
                HttpStatus.CONFLICT,
    ...
    ResponseEntity<ProblemDetail> handleUnexpected(Exception exception, HttpServletRequest request) {
        LOGGER.error("Unhandled member API failure for {}", request.getRequestURI(), exception);
        return response(
