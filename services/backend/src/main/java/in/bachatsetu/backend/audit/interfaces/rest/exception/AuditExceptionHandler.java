package in.bachatsetu.backend.audit.interfaces.rest.exception;
...
/** Converts Audit REST boundary failures to RFC 7807 problem details. */
@RestControllerAdvice(basePackages = "in.bachatsetu.backend.audit.interfaces.rest")
public class AuditExceptionHandler {
    ...
    ResponseEntity<ProblemDetail> handleNotFound(AuditEntryNotFoundException exception, HttpServletRequest request) {
    ...
    ResponseEntity<ProblemDetail> handleUnexpected(Exception exception, HttpServletRequest request) {
        LOGGER.error("Unhandled audit API failure for {}", request.getRequestURI(), exception);
        return response(
