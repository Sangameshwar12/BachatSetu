package in.bachatsetu.backend.group.interfaces.rest.exception;
...
/** Converts Savings Group REST boundary failures to RFC 7807 problem details. */
@RestControllerAdvice(basePackages = "in.bachatsetu.backend.group.interfaces.rest")
public class SavingsGroupExceptionHandler {

    private static final Logger LOGGER = LoggerFactory.getLogger(SavingsGroupExceptionHandler.class);
    ...
    ResponseEntity<ProblemDetail> handleDuplicateCode(
            DuplicateGroupCodeException exception,
            HttpServletRequest request) {
        return response(
                HttpStatus.CONFLICT,
                "group-code-conflict",
    ...
    ResponseEntity<ProblemDetail> handleUnexpected(Exception exception, HttpServletRequest request) {
        LOGGER.error("Unhandled savings group API failure for {}", request.getRequestURI(), exception);
        return response(
