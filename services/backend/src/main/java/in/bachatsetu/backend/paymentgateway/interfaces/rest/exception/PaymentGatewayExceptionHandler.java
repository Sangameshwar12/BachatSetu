package in.bachatsetu.backend.paymentgateway.interfaces.rest.exception;
...
/** Converts Payment Gateway REST boundary failures to RFC 7807 problem details. */
@RestControllerAdvice(basePackages = "in.bachatsetu.backend.paymentgateway.interfaces.rest")
public class PaymentGatewayExceptionHandler {

    private static final Logger LOGGER = LoggerFactory.getLogger(PaymentGatewayExceptionHandler.class);
    ...
    ResponseEntity<ProblemDetail> handleInvalidSignature(
            InvalidWebhookSignatureException exception, HttpServletRequest request) {
        return response(
                HttpStatus.UNAUTHORIZED, "invalid-webhook-signature", "Invalid webhook signature",
                exception.getMessage(), request);
    }

    @ExceptionHandler({GatewayOrderNotFoundException.class, PaymentNotFoundException.class})
    ResponseEntity<ProblemDetail> handleNotFound(RuntimeException exception, HttpServletRequest request) {
        return response(HttpStatus.NOT_FOUND, "not-found", "Resource not found", exception.getMessage(), request);
    }

    @ExceptionHandler({
        RefundNotAllowedException.class, AmountMismatchException.class, UnsupportedGatewayException.class,
        DomainException.class
    })
    ResponseEntity<ProblemDetail> handleBusinessFailure(RuntimeException exception, HttpServletRequest request) {
        return response(
                HttpStatus.UNPROCESSABLE_ENTITY, "payment-gateway-validation-failed", "Business validation failed",
                exception.getMessage(), request);
    }
    ...
    ResponseEntity<ProblemDetail> handleUnexpected(Exception exception, HttpServletRequest request) {
        LOGGER.error("Unhandled payment gateway API failure for {}", request.getRequestURI(), exception);
        return response(
