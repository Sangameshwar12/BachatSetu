package in.bachatsetu.backend.admin.interfaces.rest.exception;
...
/** Converts Admin REST boundary failures to RFC 7807 problem details. */
@RestControllerAdvice(basePackages = "in.bachatsetu.backend.admin.interfaces.rest")
public class AdminExceptionHandler {
    ...
    ResponseEntity<ProblemDetail> handleNotFound(PlatformUserNotFoundException exception, HttpServletRequest request) {
    ...
    /**
     * Normally intercepted upstream by {@code ExceptionTranslationFilter} and the application's shared
     * {@code AccessDeniedHandler} before reaching any controller advice; handled here too so a
     * platform-administrator-only rejection always resolves to a consistent RFC 7807 body regardless of
     * where in the filter chain it is caught.
     */
    ...
    ResponseEntity<ProblemDetail> handleUnexpected(Exception exception, HttpServletRequest request) {
        LOGGER.error("Unhandled admin API failure for {}", request.getRequestURI(), exception);
        return response(
