...
/**
 * Simulates the Stripe PaymentIntents API. **No Stripe SDK or network call is used**; see
 * {@link SimulatedRazorpayGatewayAdapter}'s Javadoc for why this and its sibling adapters are placeholders.
 */
public final class SimulatedStripeGatewayAdapter implements PaymentGatewayPort {

    private static final Logger LOGGER = LoggerFactory.getLogger(SimulatedStripeGatewayAdapter.class);
    ...
    public PaymentOrderResult createOrder(AggregateId paymentId, Money amount) {
        ...
        String orderId = "pi_" + UUID.randomUUID().toString().replace("-", "");
        String paymentLink = "https://checkout.stripe.example/pay/" + orderId;
        LOGGER.info("Simulated Stripe payment intent {} created for payment {}", orderId, paymentId);
        return new PaymentOrderResult(paymentId.value(), GatewayType.STRIPE, orderId, paymentLink);
    }
    ...
    public PaymentStatusResult fetchStatus(AggregateId paymentId, String providerOrderId) {
        ...
        LOGGER.info("Simulated Stripe status fetch for payment intent {}", providerOrderId);
        return new PaymentStatusResult(
                paymentId.value(), GatewayType.STRIPE, providerOrderId, "succeeded", true, false);
    }
}
