package in.bachatsetu.backend.receipt.interfaces.rest.exception;
...
/** Converts Receipt REST boundary failures to RFC 7807 problem details. */
@RestControllerAdvice(basePackages = "in.bachatsetu.backend.receipt.interfaces.rest")
public class ReceiptExceptionHandler {
    ...
    ResponseEntity<ProblemDetail> handleReceiptNotFound(
    ...
    ResponseEntity<ProblemDetail> handleUnexpected(Exception exception, HttpServletRequest request) {
        LOGGER.error("Unhandled receipt API failure for {}", request.getRequestURI(), exception);
        return response(
