package in.bachatsetu.backend.support.interfaces.rest.exception;
...
/** Converts Support module REST boundary failures to RFC 7807 problem details. */
@RestControllerAdvice(basePackages = "in.bachatsetu.backend.support.interfaces.rest")
public class SupportExceptionHandler {

    private static final Logger LOGGER = LoggerFactory.getLogger(SupportExceptionHandler.class);
    ...
    @ExceptionHandler(SupportApplicationException.class)
    ResponseEntity<ProblemDetail> handleSupportFailure(SupportApplicationException exception, HttpServletRequest request) {
        return switch (exception.reason()) {
            case TICKET_NOT_FOUND -> response(
                    HttpStatus.NOT_FOUND, "ticket-not-found", "Ticket not found", exception.getMessage(), request);
        };
    }
    ...
    ResponseEntity<ProblemDetail> handleDomainFailure(DomainException exception, HttpServletRequest request) {
        return response(
                HttpStatus.UNPROCESSABLE_ENTITY, "ticket-validation-failed", "Business validation failed",
    ...
    ResponseEntity<ProblemDetail> handleUnexpected(Exception exception, HttpServletRequest request) {
        LOGGER.error("Unhandled support API failure for {}", request.getRequestURI(), exception);
        return response(
