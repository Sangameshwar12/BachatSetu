package in.bachatsetu.backend.platformoperations.interfaces.rest.exception;
...
/** Converts Platform Operations module REST boundary failures to RFC 7807 problem details. */
@RestControllerAdvice(basePackages = "in.bachatsetu.backend.platformoperations.interfaces.rest")
public class PlatformOperationsExceptionHandler {

    private static final Logger LOGGER = LoggerFactory.getLogger(PlatformOperationsExceptionHandler.class);
    ...
    @ExceptionHandler(PlatformOperationsApplicationException.class)
    ResponseEntity<ProblemDetail> handlePlatformOperationsFailure(
            PlatformOperationsApplicationException exception, HttpServletRequest request) {
        return switch (exception.reason()) {
            case TENANT_NOT_FOUND -> response(
                    HttpStatus.NOT_FOUND, "tenant-not-found", "Tenant not found", exception.getMessage(), request);
        };
    }
    ...
    ResponseEntity<ProblemDetail> handleDomainFailure(DomainException exception, HttpServletRequest request) {
        return response(
                HttpStatus.UNPROCESSABLE_ENTITY, "platform-operations-validation-failed",
                "Business validation failed", exception.getMessage(), request);
    }
    ...
    ResponseEntity<ProblemDetail> handleAuthorizationDenied(HttpServletRequest request) {
        return response(
                HttpStatus.FORBIDDEN, "platform-administrator-required", "Platform administrator role required",
                "This action requires the platform administrator role.", request);
    }

    @ExceptionHandler(Exception.class)
    ResponseEntity<ProblemDetail> handleUnexpected(Exception exception, HttpServletRequest request) {
        LOGGER.error("Unhandled platform operations API failure for {}", request.getRequestURI(), exception);
        return response(
