...
class LayerDependencyArchitectureTest {
    ...
    @ArchTest
    static final ArchRule GROUP_INFRASTRUCTURE_MAY_DEPEND_ONLY_ON_OWNED_PORTS_AND_DOMAIN = classes()
            .that().resideInAnyPackage(ArchitecturePackages.GROUP_INFRASTRUCTURE)
            .should().onlyDependOnClassesThat().resideInAnyPackage(
                    ArchitecturePackages.GROUP_INFRASTRUCTURE,
                    ArchitecturePackages.GROUP_APPLICATION_PORT,
                    "..group.domain..",
                    "..shared..",
                    "java..",
                    "org.springframework..")
            .because("Savings Group infrastructure adapters implement only their owned outbound ports");
