...
class DrawInfrastructureAdapterTest {

    private static final Instant NOW = Instant.parse("2026-07-07T08:00:00Z");

    @Test
    void systemClockDelegatesToInjectedJavaClock() {
        SystemDrawClockAdapter adapter = new SystemDrawClockAdapter(Clock.fixed(NOW, ZoneOffset.UTC));
    ...
    void transactionAdapterExecutesOperationThroughTransactionTemplate() {
        ...
        SpringDrawTransactionAdapter adapter = new SpringDrawTransactionAdapter(template);
    ...
    void eventPublisherAdapterPublishesEveryDomainEvent() {
        ...
        ApplicationEventDrawEventPublisherAdapter adapter =
                new ApplicationEventDrawEventPublisherAdapter(publisher);
