...
class ResolveTicketApplicationServiceTest {
    ...
    private final ResolveTicketApplicationService service =
            new ResolveTicketApplicationService(repository, eventPublisher, clock, transaction, mapper);

    @Test
    void resolvesAnExistingTicket() {
        ...
        SupportTicket ticket = SupportTicket.create(
                ticketId, AggregateId.newId(), raisedBy, TicketCategory.DRAW, TicketPriority.MEDIUM, "Subject",
        ...
        SupportTicketResult result = service.execute(new ResolveTicketCommand(ticketId, "Fixed it", admin));

        assertThat(result.status()).isEqualTo(TicketStatus.RESOLVED);
        assertThat(result.resolution()).isEqualTo("Fixed it");
