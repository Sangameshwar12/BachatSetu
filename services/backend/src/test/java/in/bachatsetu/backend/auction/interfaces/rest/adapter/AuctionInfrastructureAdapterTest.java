...
class AuctionInfrastructureAdapterTest {

    private static final Instant NOW = Instant.parse("2026-07-08T08:00:00Z");

    @Test
    void systemClockDelegatesToInjectedJavaClock() {
        SystemAuctionClockAdapter adapter = new SystemAuctionClockAdapter(Clock.fixed(NOW, ZoneOffset.UTC));
    ...
    void transactionAdapterExecutesOperationThroughTransactionTemplate() {
        ...
        SpringAuctionTransactionAdapter adapter = new SpringAuctionTransactionAdapter(template);
    ...
    void eventPublisherAdapterPublishesEveryDomainEvent() {
        ...
        ApplicationEventAuctionEventPublisherAdapter adapter =
                new ApplicationEventAuctionEventPublisherAdapter(publisher);
